...
async function wheelBottom(page, root) {
  ...
  await page.waitForTimeout(80); // Allow asynchronous wheel delivery before observing settlement.
  await root.evaluate(async el => {
    const parents = []; for (let p = el; p; p = p.parentElement) parents.push(p);
...
(async () => {
  ...
  try {
    for (const theme of ['light', 'dark']) for (const width of [450, 501, 900]) for (const view of ['Privacy', 'Setup']) {
      ...
      try {
        ...
        if (privacyStyle && view === 'Privacy') await page.addStyleTag({ content: privacyStyle });
